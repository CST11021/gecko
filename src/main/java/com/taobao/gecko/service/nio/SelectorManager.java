...
/**
 * SelectorManager，用于管理多个reactor，当SelectorManager启动时，会启动所有的Reactor实例：
 *
 * SelectorManager对外提供了2个能力：
 * 1、注册了 SelectableChannel 到 Selector
 * 2、注册Session相关事件
 *
 * 以上两个功能内部通过Reactor来实现，并触发相关的事件监听
 *
 * @author boyan
...
public class SelectorManager {
    ...
    /**
     * 表示准备就绪的个数，当Reactor线程启动的时候，会使 reactorReadyCount + 1
     */
    ...
    public final Reactor registerChannel(final SelectableChannel channel, final int ops, final Object attachment) {
        ...
        // 确定当前的reactor实例：OP_ACCEPT（服务端接收连接请求）和OP_CONNECT（客户端发起连接请求）单独使用一个Reactor
        int index = 0;
     ...
     * @param event
     */
    public final void registerSession(final Session session, final SessionEventType event) {
     ...
     * @return
     */
    private final Reactor nextReactor() {
        if (this.dividend > 0) {
     ...
     * @return
     */
    public Reactor getReactorFromSession(final Session session) {
    ...
    /**
     * Reactor线程即将开始执行前调用该方法，让其他相关组件做好准备
     */
     ...
     * @return
     */
    public Reactor getReactorByIndex(final int index) {
