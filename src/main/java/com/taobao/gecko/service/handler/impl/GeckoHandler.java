 ...
 */
package com.taobao.gecko.service.handler.impl;

import com.taobao.gecko.remoting.DefaultRemotingContext;
import com.taobao.gecko.service.connection.*;
import com.taobao.gecko.remoting.RemotingController;
...
import com.taobao.gecko.exception.IllegalMessageException;
...
public class GeckoHandler implements Handler {
    ...
    private <T extends RequestCommand> RequestProcessor<T> getProcessorByMessage(final Object message) {
        ...
        if (message instanceof HeartBeatRequestCommand) {
            processor = (RequestProcessor<T>) this.remotingContext.getProcessorMap().get(HeartBeatRequestCommand.class);
        } else {
            processor = (RequestProcessor<T>) this.remotingContext.getProcessorMap().get(message.getClass());
        }
        return processor;
    }
    ...
    private void notifyConnectionReady(final DefaultConnection conn) {
        ...
        if (conn != null) {
            ...
            // 通知监听器连接就绪
            for (final ConnectionLifeCycleListener listener : this.remotingContext.getConnectionLifeCycleListenerList()) {
                try {
