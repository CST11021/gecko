...
import com.taobao.gecko.service.session.impl.NioTCPSession;
...
public abstract class SocketChannelController extends NioController {
    ...
    protected final void dispatchReadEvent(final SelectionKey key) {
        ...
        if (session != null) {
            // 派发IO事件
            ((NioSession) session).onEvent(SessionEventType.READABLE, key.selector());
        } else {
    ...
    protected final void dispatchWriteEvent(final SelectionKey key) {
        ...
        if (session != null) {
            // 派发IO事件
            ((NioSession) session).onEvent(SessionEventType.WRITEABLE, key.selector());
        } else {
