 ...
 */
package com.taobao.gecko.remoting;

import com.taobao.gecko.service.connection.impl.DefaultConnection;
import com.taobao.gecko.service.server.GroupManager;
...
import com.taobao.gecko.service.connection.ConnectionLifeCycleListener;
...
public class DefaultRemotingContext implements RemotingContext, DefaultRemotingContextMBean {
    ...
    public ConcurrentHashMap<Class<? extends RequestCommand>, RequestProcessor<? extends RequestCommand>> getProcessorMap() {
        return processorMap;
    }

    public CopyOnWriteArrayList<ConnectionLifeCycleListener> getConnectionLifeCycleListenerList() {
        return connectionLifeCycleListenerList;
    }
     ...
     * @param conn
     */
    public void notifyConnectionCreated(final Connection conn) {
     ...
     * @param conn
     */
    public void notifyConnectionClosed(final Connection conn) {
        for (final ConnectionLifeCycleListener listener : this.connectionLifeCycleListenerList) {
            try {
    ...
    public void addSession2ConnectionMapping(final NioSession session, final DefaultConnection conn) {
        this.session2ConnectionMap.put(session, conn);
    }

    public DefaultConnection getConnectionBySession(final NioSession session) {
        return this.session2ConnectionMap.get(session);
    }

    public DefaultConnection removeSession2ConnectionMapping(final NioSession session) {
